...
const Chatbot = () => {
    ...
    if (userInput.trim()) {
      ...
      setTimeout(() => {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { sender: "bot", text: "Here is the analysis for your query: ..." },
        ]);
      }, 2000); 
    }
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setTimeout(() => {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { sender: "bot", text: "Here is the analysis for your image: ..." },
        ]);
      }, 3000);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-screen bg-blue-50 p-6">
      <section className="w-full max-w-xl bg-white shadow-xl rounded-lg p-6">
        <header className="flex justify-between items-center pb-4">
          <div>
            <h2 className="text-lg font-bold text-blue-800">Medical Chatbot</h2>
            <p className="text-sm text-gray-600">Your personal medical assistant</p>
          </div>
          <div className="flex items-center">
            <BsFileMedical size={24} color="#1E40AF" />
            <span className="ml-2 text-blue-800">Age: 34 | Gender: Male</span>
          </div>
        </header>

        {/* Chat Messages */}
        <div className="h-72 overflow-auto bg-gray-100 p-4 rounded-lg space-y-4">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
              <div
                className={`rounded-lg p-3 max-w-xs ${msg.sender === "user" ? "bg-blue-500 text-white" : "bg-gray-200"}`}
              >
                {msg.sender === "user" ? (
